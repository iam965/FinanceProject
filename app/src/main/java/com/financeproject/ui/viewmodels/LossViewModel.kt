...
class LossViewModel(application: Application): AndroidViewModel(application) {
    ...
    class LossViewModelFactory(private val application: Application) : ViewModelProvider.Factory {
        override fun <T : ViewModel> create(modelClass: Class<T>): T {
            if (modelClass.isAssignableFrom(LossViewModel::class.java)) {
                @Suppress("UNCHECKED_CAST")
                return LossViewModel(application) as T
            }
